...
use crate::{
    ...
    models::{Assignment, CanvasFile, CanvasProfile, Course, TodoItem},
};
...
impl CanvasClient {
    ...
    pub async fn todos(&self) -> AppResult<Vec<TodoItem>> {
        let values: Vec<Value> = self.get_json("/api/v1/users/self/todo", &[]).await?;
        Ok(values.into_iter().filter_map(todo_from_value).collect())
    }
    ...
    pub async fn assignments(&self, course_id: &str) -> AppResult<Vec<Assignment>> {
        validate_canvas_id(course_id)?;
        let values = self
            .paginated_values(
            ...
            .await?;
        Ok(values.iter().filter_map(assignment_from_value).collect())
    }
    ...
    pub async fn resolve_file_url(&self, file: &CanvasFile) -> AppResult<ResolvedFile> {
        ...
        for _ in 0..6 {
            if !same_origin(&current, &canvas_origin) {
                direct_url = Some(current.to_string());
                break;
            }
            let response = self
                .no_redirect
...
fn todo_from_value(value: Value) -> Option<TodoItem> {
    let assignment = value.get("assignment")?;
    Some(TodoItem {
        id: value_id(assignment, "id")?,
        title: optional_string(assignment, "name")
            .or_else(|| optional_string(&value, "title"))
            .unwrap_or_else(|| "未命名任务".into()),
        course_name: optional_string(&value, "context_name").unwrap_or_default(),
        due_at: optional_string(assignment, "due_at"),
        points_possible: assignment.get("points_possible").and_then(Value::as_f64),
        submitted: value
            .get("ignore")
...
fn assignment_from_value(value: &Value) -> Option<Assignment> {
    let submission_state = value
        .get("submission")
        .and_then(|submission| optional_string(submission, "workflow_state"))
        .unwrap_or_else(|| "unsubmitted".into());
    Some(Assignment {
        id: value_id(value, "id")?,
        name: optional_string(value, "name").unwrap_or_else(|| "未命名作业".into()),
        due_at: optional_string(value, "due_at"),
        points_possible: value.get("points_possible").and_then(Value::as_f64),
        submission_state,
    })
}
